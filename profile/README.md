# tokio-drift

This organization is a collection of all the major group projects we have worked on during our B. Tech in Computer Science. Since every member is not involved in every project, collaborators will be listed individually for each repository.

---

## Projects

## MOSS

**High-performance T20 cricket simulator in C, using POSIX threads**  

Maps cricket entities to OS concepts - the players are threads, the pitch a critical section.  
A custom priority scheduler to schedule bowlers along with the traditional Round Robin and SJF options.  

Lang: C  

[![View Repo](https://img.shields.io/badge/View%20Repository-orange?logo=github)](https://github.com/tokio-drift/MOSS)

Collaborators: [peopleig](https://github.com/peopleig), [KNIGHT-29](https://github.com/KNIGHT-29), [ParshvMeshiya](https://github.com/ParshvMeshiya), [HetB180906](https://github.com/HetB180906)

## Hyperion

**Feature Packed Image Editor that runs on the browser**  

Browser-based photo editor built with React.  
Supports global tonal and color adjustments, selective local masking, crop/rotate tools, multi-image workflow, and export in common web formats.

Lang: JavaScript  

[![View Repo](https://img.shields.io/badge/View%20Repository-red?logo=github)](https://github.com/tokio-drift/Hyperion)

Collaborators: [peopleig](https://github.com/peopleig), [ParshvMeshiya](https://github.com/ParshvMeshiya), [HetB180906](https://github.com/HetB180906), [Varun-bhim](https://github.com/Varun-bhim), [YagnikMaru](https://github.com/YagnikMaru), [Jheelgandhi9](https://github.com/Jheelgandhi9)

## Apollo  

**Compiler for a C-like language, built on MIPS with 3AC**  

Lang: C++ 

Collaborators: [peopleig](https://github.com/peopleig), [ParshvMeshiya](https://github.com/ParshvMeshiya), [HetB180906](https://github.com/HetB180906), [KNIGHT-29](https://github.com/KNIGHT-29)
